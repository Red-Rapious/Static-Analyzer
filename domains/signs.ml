...
module SignsDomain = struct
  ...
  let unary x = function
    | AST_UNARY_PLUS -> x
    | AST_UNARY_MINUS -> (
        match x with Top | Bot | Null -> x | Pos -> Neg | Neg -> Pos)
  ...
  *)
  let rec compare x y op =
    ...
    | AST_LESS -> (
        match (x, y) with
        | Top, Top -> (Top, Top)
        | Null, Neg | Pos, Neg | Pos, Null | _, _ -> (x, y))
    | AST_LESS_EQUAL -> (
        match (x, y) with
        | Top, Top -> (Top, Top)
        ...
        | _, _ -> (x, y))
    | AST_GREATER ->
        let x', y' = compare y x AST_LESS in
        (y', x')
    | AST_GREATER_EQUAL ->
        let x', y' = compare y x AST_LESS_EQUAL in
        (y', x')
  ...
  *)
  ...
  let subset x y =
    match (x, y) with Bot, _ -> true | _, Top -> true | _ -> x = y
