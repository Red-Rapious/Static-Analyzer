...
module ConstantDomain : VALUE_DOMAIN = struct
  ...
  let unary x = function
    | AST_UNARY_PLUS -> x
    | AST_UNARY_MINUS -> (
        match x with Top | Bot -> x | Int x -> Int (Z.neg x))
  ...
  let binary x y op =
    ...
    | Top, _, _ -> Top
    (* division by zero *)
    | _, Int z, AST_DIVIDE when z = Z.zero ->
        Bot
        (*| _, Top, AST_DIVIDE -> Bot (* division by zero may occur *)*)
        (* TODO?? *)
  ...
  *)
  let bwd_binary x y op r =
    ...
    | AST_MULTIPLY ->
        let aux x y r =
          ...
          | Int z, Int z' when z = Z.zero && z' = Z.zero -> x
          (* this can't happen since _ * 0 = 0 *)
          | Int z, Int _ when z = Z.zero -> Bot
          (* default case *)
  ...
  let print formatter t =
    Format.fprintf formatter "%s"
      (match t with Top -> "⊤" | Bot -> "⊥" | Int i -> Z.to_string i)
end
