...
module KarrDomain (Vars : VARS) : DOMAIN =
functor
  (_ : Value_domain.VALUE_DOMAIN)
  ->
  struct
    type t = constraints
    ...
    let print formatter d =
      ...
      | Constraints c ->
          let rec aux index const = function
            | [] -> ()
            | [ h ] ->
                Format.fprintf formatter "%s * %s"
                  (Q.to_string const.(index))
                  h.var_name
            | h :: t ->
                Format.fprintf formatter "%s" (Q.to_string const.(index));
                Format.fprintf formatter " * %s + " h.var_name;
                aux (index + 1) const t
          in
    ...
    let init =
      ...
      in
      for i = 0 to Array.length c - 1 do
        c.(i) <- (Q.zero, Array.make (List.length Vars.support) Q.zero)
      done;
      for i = 0 to Array.length c - 1 do
        (snd c.(i)).(i) <- Q.one
      done;
      ...
      | Constraints c ->
          ...
          if !index < 0 then top
            ...
            for i = 0 to Array.length res - 1 do
              res.(i) <- (Q.zero, Array.make (List.length Vars.support) Q.zero)
            done;
            for i = 0 to !index do
              res.(i) <- c.(i)
            done;
    ...
    let is_bottom = function t -> is_eq (Bot, t)
    ...
    let meet c1 c2 =
      ...
      | Constraints c1, Constraints c2 ->
          ...
          in
          for i = 0 to Array.length res - 1 do
            res.(i) <- (Q.zero, Array.make (Array.length c1) Q.zero)
          done;
          for l1 = 0 to Array.length c1 - 1 do
            res.(l1) <- c1.(l1)
          done;
          for l1 = 0 to Array.length c2 - 1 do
            res.(l1 + Array.length c1) <- c2.(l1)
          done;
    ...
    let assign domain var e =
      if not (is_affine e) then top
        ...
        let index = var.var_id - 1 in
        match (affinify_int e, domain) with
        ...
        | _, Bot -> Bot
        | None, _ -> Bot
        | Some c, Constraints d when Q.( = ) Q.zero (snd c).(index) ->
            for i = 0 to Array.length d - 1 do
              if Q.( = ) Q.zero (snd d.(i)).(index) then ()
              else
                let ai = (snd d.(i)).(index) in
                let const =
                  (Q.( * ) ai (fst c), Array.map (Q.( * ) ai) (snd c))
                in
                d.(i) <-
                  ( Q.( + ) (fst const) (fst d.(i)),
        ...
        | Some c, Constraints d ->
            let lv = (snd c).(index) in
            let const =
              ( Q.( ~- ) (Q.( / ) (fst c) lv),
                Array.make (Array.length (snd c)) Q.zero )
            in
            for k = 0 to Array.length (snd c) - 1 do
              if k <> index then
                (snd const).(k) <- Q.( ~- ) (Q.( / ) (snd c).(k) lv)
              else (snd const).(k) <- Q.( / ) Q.(one) lv
            done;
            for i = 0 to Array.length d - 1 do
              if Q.( = ) Q.zero (snd d.(i)).(index) then ()
              else
                let ai = (snd d.(i)).(index) in
                let const =
                  (Q.( * ) ai (fst const), Array.map (Q.( * ) ai) (snd const))
                in
                d.(i) <-
                  ( Q.( + ) (fst const) (fst d.(i)),
                    Array.map2 Q.( + ) (snd const) (snd d.(i)) )
            done;
            simplify (Constraints d)
    ...
    let join d1 d2 =
      ...
      | Constraints d1, Constraints d2 ->
          let c1 =
            Array.make_matrix (Array.length d1)
              (Array.length (snd d1.(0)) + 1)
              Q.zero
          and c2 =
            Array.make_matrix (Array.length d2)
              (Array.length (snd d2.(0)) + 1)
              Q.zero
          in
          ...
          for s = 0 to Array.length c1 - 1 do
            if Q.( = ) c1.(!r).(s) Q.one && Q.( = ) c2.(!r).(s) Q.one then
              incr r;
            if Q.( = ) c1.(!r).(s) Q.one && Q.( = ) c2.(!r).(s) Q.zero then (
              for i = 0 to !r - 2 do
                c1.(i) <-
                  Array.map2 Q.( + )
                    (Array.map (fun x -> Q.( * ) c2.(i).(s) x) c1.(!r))
                    c1.(i)
              done;
              ...
              c1.(Array.length c1 - 1) <-
                Array.make (Array.length c1.(0)) Q.zero);
            if Q.( = ) c2.(!r).(s) Q.one && Q.( = ) c1.(!r).(s) Q.zero then (
              for i = 0 to !r - 2 do
                c2.(i) <-
                  Array.map2 Q.( + )
                    (Array.map (fun x -> Q.( * ) c1.(i).(s) x) c2.(!r))
                    c2.(i)
              done;
              for j = Array.length c2 - 2 downto !r do
                c2.(j) <- Array.copy c2.(j + 1)
              done;
              c2.(Array.length c2 - 1) <-
                Array.make (Array.length c2.(0)) Q.zero);
            if Q.( = ) c1.(!r).(s) Q.zero && Q.( = ) c2.(!r).(s) Q.zero then (
              let t = ref (!r - 1) and tmp = ref true in
              while !tmp && !t >= 0 do
                if Q.( = ) c1.(!t).(s) c2.(!t).(s) then decr t else tmp := false
              done;
              if not !tmp then ()
              else
                for i = 0 to !t - 1 do
                  ...
                  c1.(i) <-
                    Array.map2 Q.( - ) c1.(i)
                      (Array.map (Q.( * ) coeff) c1.(!t));
                  c2.(i) <-
                    Array.map2 Q.( - ) c2.(i)
                      (Array.map (Q.( * ) coeff) c2.(!t));
    ...
    let rec constraintify = function
      | CFG_bool_const v -> top (*TODO: Propager la valeur*)
      | CFG_bool_unary (AST_NOT, e) -> (
          match constraintify e with Bot -> Bot | v -> v)
    ...
    let guard domain e =
      if not (is_affine_bool e) then domain
      else
