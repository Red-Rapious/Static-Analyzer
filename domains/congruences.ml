...
module CongruencesDomain : Value_domain.VALUE_DOMAIN =
  struct
    ...
    *)
    ...
    | AST_UNARY_MINUS -> match x with
                         | Bot -> Bot
                         | Modulo (a, b) -> Modulo(a, Z.neg b)

    (* binary operation *)
    let binary x y op = 
      match x, y with
      | Bot, _ | _, Bot -> Bot
      | Modulo(a1, b1), Modulo(a2, b2) -> match op with
                                          | AST_PLUS -> Modulo(Z.gcd a1 a2, Z.add b1 b2)
                                          | AST_MINUS -> Modulo(Z.gcd a1 a2, Z.sub b1 b2)
                                          | AST_MULTIPLY -> Modulo(Z.gcd (Z.mul a1 a2) (Z.gcd (Z.mul a2 b1) (Z.mul a1 b2)) , Z.mul b1 b2)
                                          | AST_DIVIDE when a2 = Z.zero -> 
                                            if b2 = Z.zero then Bot (* division by zero *)
                                            else if a1 = Z.zero then Modulo(Z.zero, Z.div b1 b2) (* constant divided by a constant *)
    ...
    let join x y = match x, y with
                   | _, Bot -> x
                   | Bot, _ -> y
                   | Modulo(a1, b1), Modulo(a2, b2) -> Modulo(Z.gcd (Z.gcd a1 a2) (Z.sub b1 b2), b1)

    let meet x y = match x, y with
                   | _, Bot
                   | Bot, _ -> Bot
                   | Modulo(a1, b1), Modulo(a2, b2) -> let (g, s, _) = Z.gcdext a1 a2 in
                                                       if not (Z.congruent b1 b1 g) then Bot
                                                       else if g = Z.zero then Modulo (Z.zero, b1)
                                                            else Modulo (
                                                              Z.lcm a1 a2, 
                                                              Z.add (Z.mul (Z.sub b2 b1) (Z.mul s (Z.div a1 g))) 
                                                                     b1)
