...
module type DOMAIN = functor
  (_ : VALUE_DOMAIN)
  -> (* functor as suggested by the subject *)
sig
...
module Domain (Vars : VARS) : DOMAIN =
functor
  ...
  struct
    ...
    *)
    ...
    let init =
      List.fold_left
        (fun map var -> VarMap.add var (ValueDomain.const Z.zero) map)
        VarMap.empty Vars.support

    (* empty set of environments *)
    let bottom =
      List.fold_left
        (fun map var -> VarMap.add var ValueDomain.bottom map)
        VarMap.empty Vars.support
    ...
    let rec int_expr_to_value domain = function
      ...
      | CFG_int_binary (op, e1, e2) ->
          ValueDomain.binary
            (int_expr_to_value domain e1)
            (int_expr_to_value domain e2)
            op
    ...
    *)
    let rec bwd domain expr final =
      match expr with
      (* if the constant is in the value domain, then nothing changed, otherwise it cannot happen, hence bottom *)
      | CFG_int_const a ->
          if ValueDomain.subset (ValueDomain.const a) final then domain
          else bottom
      | CFG_int_rand (a, b) ->
          (* if the meeting of the random domain and the final is bottom,
             then no possible original domain can achieve this *)
          if
            ValueDomain.is_bottom
              (ValueDomain.meet final (ValueDomain.rand a b))
          then bottom (* otherwise, everything's good *)
          else domain
      (* propagate *)
      | CFG_int_unary (op, expr) ->
          bwd domain expr
            (ValueDomain.bwd_unary (int_expr_to_value domain expr) op final)
      (* propagate into the graph *)
      | CFG_int_binary (op, e1, e2) ->
          let final1, final2 =
            ValueDomain.bwd_binary
              (int_expr_to_value domain e1)
              (int_expr_to_value domain e2)
              op final
          in
      ...
      | CFG_int_var var ->
          let value =
            match VarMap.find_opt var domain with
            | Some x -> x
            | None -> ValueDomain.bottom
            (* TODO: this is a temporary fix, I suppose? *)
          in
    ...
    let rec guard domain = function
      ...
      | CFG_bool_binary (AST_AND, b1, b2) ->
          meet (guard domain b1) (guard domain b2)
      | CFG_bool_binary (AST_OR, b1, b2) ->
          join (guard domain b1) (guard domain b2)
    ...
    let subset dom1 dom2 =
      VarMap.fold
        (fun var value is_subset ->
          if VarMap.mem var dom2 then
            ValueDomain.subset value (VarMap.find var dom2) && is_subset
          else false)
        dom1 true
    ...
    let widen dom1 dom2 =
      if subset dom1 dom2 then
        ...
        VarMap.mapi
          (fun var _ ->
            let value =
              match VarMap.find_opt var dom1 with
              | Some x -> x
              | None -> ValueDomain.bottom
            in
