...
let[@warning "-8"] rec div_bound x y =
  ...
  | _, Finite z when Z.equal z Z.zero ->
      failwith "interval.div_bound: division by zero"
...
module IntervalDomain = struct
  ...
  let print formatter = function
    ...
    | Interval (a, b) ->
        Format.fprintf formatter "[%s, %s]@." (bound_to_string a)
          (bound_to_string b)
  ...
  let meet x y =
    ...
    | Interval (a, b), Interval (c, d) ->
        bottomize_if_necessary (Interval (max_bound a c, min_bound b d))
  ...
  let join x y =
    ...
    | _, Bot -> x
    | Interval (a, b), Interval (c, d) ->
        let lb = min_bound a c and rb = max_bound b d in
        if lb = MinusInf && rb = PlusInf then Top else Interval (lb, rb)
  ...
  let binary x y op =
    ...
    | Interval (a, b), Interval (c, d), _ -> (
        ...
        | AST_MULTIPLY ->
            let b1 = mul_bound a c
            and b2 = mul_bound a d
            and b3 = mul_bound b c
            and b4 = mul_bound b d in
            let mini = min_bound (min_bound b1 b2) (min_bound b3 b4)
            and maxi = max_bound (max_bound b1 b2) (max_bound b3 b4) in
            Interval (mini, maxi)
        | AST_MODULO -> (
            match (a, b, c, d) with
        ...
        | AST_DIVIDE ->
            let naive x y =
              ...
              | Bot, _ | _, Bot -> Bot
              | Interval (a, b), Interval (c, d) ->
                  Interval (div_bound a d, div_bound b c)
            in
            let leftp =
              bottomize_if_necessary (Interval (max_bound (Finite Z.zero) a, b))
            and leftn =
              bottomize_if_necessary
                (Interval (max_bound (Finite Z.one) (neg_bound b), neg_bound a))
            and rightp =
              bottomize_if_necessary (Interval (max_bound (Finite Z.one) c, d))
            and rightn =
              bottomize_if_necessary
                (Interval (max_bound (Finite Z.one) (neg_bound d), neg_bound c))
            in
            let pos = join (naive leftp rightp) (naive leftn rightn)
            and neg =
              unary
                (join (naive leftn rightp) (naive leftp rightn))
                AST_UNARY_MINUS
            in
            join pos neg)
  ...
  *)
  let mono_compare a b c d op =
    let lu, ru =
      ...
      | AST_NOT_EQUAL ->
          if c = d && b = c then (a, sub_bound b (Finite Z.one))
          else if c = d && d = a then (add_bound a (Finite Z.one), b)
          else (a, b)
  ...
  let compare u v op =
    let res =
      match (u, v) with
      | Interval (a, b), Interval (c, d) ->
          (mono_compare a b c d op, mono_compare c d a b (ast_cop_reverse op))
      | _ -> (Bot, Bot)
    in
    res
  ...
  *)
  let bwd_binary x y op r =
    ...
    | AST_MODULO ->
        (x, y)
        (* meet x Top = x, and we can't invert modulo so we abstract the invert as Top *)
    | AST_MULTIPLY -> (
        match (x, y, r) with
        | Top, _, _ | _, Top, _ | _, _, Top -> (Top, Top)
        | Bot, _, _ | _, Bot, _ | _, _, Bot -> (Bot, Bot)
        | Interval (a, b), Interval (c, d), Interval (e, f) ->
            let contains_zero x y =
              geq_bound (Finite Z.zero) x && geq_bound y (Finite Z.zero)
            in
            let binary_div_round_up x y =
              match (x, y) with
              | Top, _ | _, Top -> Top
              | Bot, _ | _, Bot -> Bot
              | Interval (a, b), Interval (c, d) ->
                  if d = PlusInf then Interval (Finite Z.one, div_bound b c)
                    ...
                    Interval
                      ( div_bound (add_bound a (sub_bound d (Finite Z.one))) d,
                        div_bound b c )
            in

            let remainderp =
              bottomize_if_necessary (Interval (max_bound (Finite Z.one) e, f))
            and remaindern =
              bottomize_if_necessary
                (Interval (max_bound (Finite Z.one) (neg_bound f), neg_bound e))
            in
            ...
            let backward a b =
              if contains_zero a b && contains_zero e f then Top
                ...
                let pos =
                  bottomize_if_necessary
                    (Interval (max_bound (Finite Z.one) a, b))
                and neg =
                  bottomize_if_necessary
                    (Interval
                       (max_bound (Finite Z.one) (neg_bound b), neg_bound b))
                in
                join
                  (join
                     (binary_div_round_up remainderp pos)
                     (binary_div_round_up remaindern neg))
                  (unary
                     (join
                        (binary_div_round_up remaindern pos)
                        (binary_div_round_up remainderp neg))
                     AST_UNARY_MINUS)
            in
            (meet (backward c d) x, meet (backward a b) y))
    | _ ->
        ( meet x (binary r y (ast_bop_inv op)),
          meet y (binary r x (ast_bop_inv op)) )
  ...
  let widen x y =
    ...
    | Interval (a, b), Interval (c, d) ->
        Interval
          ( (if gt_bound c a then a else MinusInf),
  ...
  let is_bottom = function
    ...
    | Interval (a, b) when a > b ->
        Format.eprintf
          "[warning] interval.is_bottom: an interval with incorrect bounds has \
           been detected\n";
        true
    | _ -> false
  ...
  let to_sign = function
    ...
    | Interval (z, z') when z = Finite Z.zero && z' = Finite Z.zero ->
        SignsDomain.Null
  ...
  let to_congruences = function
    ...
    | Interval (Finite a, Finite b) when a = b ->
        CongruencesDomain.Modulo (Z.zero, a) (* constant *)
