...
module ReducedProductDomain = struct
  ...
  (* binary operation *)
  let binary (xi, xc) (yi, yc) op =
    reduction (I.binary xi yi op, C.binary xc yc op)
  ...
  *)
  let rec compare (xi, xc) (yi, yc) op =
    let i1, i2 = I.compare xi yi op and c1, c2 = C.compare xc yc op in
    (reduction (i1, c1), reduction (i2, c2))
  ...
  (* [bwd_unary x op r] return x':
     ...
     the operation on x
  *)
  let bwd_unary (i, c) op (ri, rc) =
    reduction (I.bwd_unary i op ri, C.bwd_unary c op rc)
  ...
  *)
  let bwd_binary (xi, xc) (yi, yc) op (ri, rc) =
    ...
    and c1, c2 = C.bwd_binary xc yc op rc in
    (reduction (i1, c1), reduction (i2, c2))
