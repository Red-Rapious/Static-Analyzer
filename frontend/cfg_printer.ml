  ...
  if p.pos_fname = q.pos_fname then
    if p.pos_lnum = q.pos_lnum then
      if p.pos_cnum = q.pos_cnum then
        ...
        Format.sprintf "%s:%i.%i-%i" p.pos_fname p.pos_lnum
          (p.pos_cnum - p.pos_bol) (q.pos_cnum - q.pos_bol)
    else
      Format.sprintf "%s:%i.%i-%i.%i" p.pos_fname p.pos_lnum
        (p.pos_cnum - p.pos_bol) q.pos_lnum (q.pos_cnum - q.pos_bol)
  else
    Format.sprintf "%s:%i.%i-%s:%i.%i" p.pos_fname p.pos_lnum
...
let string_of_bool_unary_op = function AST_NOT -> "!"
...
let string_of_bool_binary_op = function AST_AND -> "&&" | AST_OR -> "||"
...
let print_option f none fmt l =
  match l with None -> Format.fprintf fmt "%s" none | Some v -> f fmt v

(* expressions *)
(* *********** *)

let print_var fmt v = Format.fprintf fmt "%s(%i)" v.var_name v.var_id
let string_of_type t = match t with AST_TYP_INT -> "int"

let rec print_int_expr fmt e =
  match e with
  | CFG_int_unary (op, e1) ->
      Format.fprintf fmt "%s" (string_of_int_unary_op op);
      if int_expr_precedence e1 <= int_expr_precedence e then
        Format.fprintf fmt " (%a)" print_int_expr e1
  ...
  | CFG_int_binary (op, e1, e2) ->
      ...
      if int_expr_precedence e2 <= int_expr_precedence e then
        Format.fprintf fmt " (%a)" print_int_expr e2
...
and print_bool_expr fmt e =
  ...
  | CFG_bool_unary (op, e1) ->
      ...
      if bool_expr_precedence e1 <= bool_expr_precedence e then
        Format.fprintf fmt " (%a)" print_bool_expr e1
  ...
  | CFG_bool_binary (op, e1, e2) ->
      ...
      if bool_expr_precedence e2 <= bool_expr_precedence e then
        Format.fprintf fmt " (%a)" print_bool_expr e2
      else Format.fprintf fmt " %a" print_bool_expr e2
  | CFG_compare (op, e1, e2) ->
      if int_expr_precedence e1 < bool_expr_precedence e then
        Format.fprintf fmt "(%a) " print_int_expr e1
      else Format.fprintf fmt "%a " print_int_expr e1;
      Format.fprintf fmt "%s" (string_of_compare_op op);
      if int_expr_precedence e2 <= bool_expr_precedence e then
        Format.fprintf fmt " (%a)" print_int_expr e2
      else Format.fprintf fmt " %a" print_int_expr e2
...
let print_inst fmt i =
  ...
  | CFG_assign (v, e) ->
      Format.fprintf fmt "%a = %a" print_var v print_int_expr e
...
let print_cfg fmt p =
  let pp_var fmt v =
    Format.fprintf fmt "%s(%i):%s" v.var_name v.var_id
      (string_of_type v.var_type)
  in
  ...
  List.iter
    (fun f ->
      Format.fprintf fmt "  %i: %s(%a) -> %a at %s, entry: %i, exit: %i, calls:"
        f.func_id f.func_name (print_list pp_var ",") f.func_args
        (print_option pp_var "void")
        f.func_ret
  ...
  List.iter
    (fun n ->
      Format.fprintf fmt "  %i: at %s, in: " n.node_id
        (string_of_position n.node_pos);
  ...
  List.iter
    (fun a ->
      Format.fprintf fmt "  %i -> %i: %a\n" a.arc_src.node_id a.arc_dst.node_id
        print_inst a.arc_inst)
    p.cfg_arcs;
  Format.fprintf fmt "\n"
  ...
  List.iter
    (fun a ->
      Format.fprintf fmt " %i -> %i [label=\"%a\"];\n" a.arc_src.node_id
        a.arc_dst.node_id print_inst a.arc_inst)
    p.cfg_arcs;
  let isguard arc =
    match arc.arc_inst with CFG_guard _ -> true | _ -> false
  in
  List.iter
    (fun n ->
      if n.node_out <> [] && List.for_all isguard n.node_out then
  ...
  List.iter
    (fun f ->
      Format.fprintf fmt "  entry_%s [shape=box,label=\"%s(%a) -> %a\"];\n"
        f.func_name f.func_name (print_list print_var ",") f.func_args
        (print_option print_var "void")
        f.func_ret;
