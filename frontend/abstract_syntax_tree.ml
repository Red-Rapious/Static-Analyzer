...
type typ = (* mathematical integers, in Z *)
  | AST_TYP_INT
...
type int_expr =
  ...
  | AST_int_rand of string ext * (* lower bound *) string ext (* upper bound *)
  (* calls a function with arguments and return value *)
  | AST_expr_call of id ext * (* function name *) int_expr ext list
(* arguments *)
...
type stat =
  ...
  | AST_stat_call of id ext * (* function name *) int_expr ext list
  (* arguments *)
...
and var_decl = typ ext * (* type *) var_init list
(* variable list *)

(* each declared variable has an optional initializer *)
and var_init = id ext * (* declared variable *) int_expr ext option
(* initializer *)
