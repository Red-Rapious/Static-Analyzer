...
and bool_expr (env : env) (expr : Abstract_syntax_tree.bool_expr) :
  ...
  | AST_bool_const f -> (env, [], CFG_bool_const f)
  | AST_bool_rand -> (env, [], CFG_bool_rand)

(* Translate a call. *)

and call (env : env) ((id, x) : id ext)
    (exprs : Abstract_syntax_tree.int_expr ext list) :
...
let prog ((t, x) : prog) : cfg =
  ...
  let env, revinit =
    List.fold_left
      (fun (env, revinit) t ->
        match t with
        | AST_fun_decl (f, _) -> (func env f, revinit)
        | AST_global_decl (d, _) ->
