...
functor
  ...
  struct
    ...
    let rec iter_arc environment arc : D.t =
      let domain = environment_of_node environment arc.arc_src in
      ...
      | CFG_assert (expr, extent) ->
          ...
          and false_domain = D.guard domain (CFG_bool_unary (AST_NOT, expr)) in

          if not (D.is_bottom false_domain) then (
            (* TODO: clean error handling *)
            Format.printf "%a: %s \"%a\"@." Cfg_printer.pp_pos (fst extent)
              "Assertion failure" Cfg_printer.print_bool_expr expr;
            if !Options.verbose then (
              Format.printf
                "The assertion is false when the value is in the following \
                 non-bottom domain:\n";
              D.print Format.std_formatter false_domain;
              Format.printf "@."))
          else if !Options.verbose then (
  ...
  *)
    and iterate_function environment (func : func) (entry_node : node option)
        entry_domain =
      (* TODO: replace node option by node; to do so, change calls using None to calls using func.func_entry *)
      let entry =
        match entry_node with Some x -> x | None -> func.func_entry
      in
      ...
      while not (NodeSet.is_empty !worklist) do
        ...
        let sources =
          List.filter
            (fun arc -> Hashtbl.mem environment arc.arc_src)
            node.node_in
        in
        let sources_domains =
          List.map (fun arc -> iter_arc environment arc) sources
        in
        let join_domain = List.fold_left D.join D.bottom sources_domains in

        (* if the node’s abstract value has changed *)
        if
          Hashtbl.mem environment node |> not
          && join_domain <> environment_of_node environment node
        then (
          let widen_if_necessary =
            if Hashtbl.mem widen_next node then
              D.widen (environment_of_node environment node) join_domain
            else join_domain
          in
          ...
          List.iter
            (fun arc -> worklist := NodeSet.add arc.arc_dst !worklist)
            node.node_out)
      done;
